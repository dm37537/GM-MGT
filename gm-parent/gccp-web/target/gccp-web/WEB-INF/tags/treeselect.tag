...
<div class="input-append">
	...
	<input id="${id}Name" name="${labelName}" readonly="readonly" type="text" value="<m:message bundle="common" key="${labelValue}"/>" maxlength="50"${disabled eq "true"? " disabled=\"disabled\"":""}"
...
<script type="text/javascript">
	$("#${id}Button").click(function(){
		...
		// 正常打开	
		top.$.jBox.open("iframe:${ctx}/tag/treeselect?url="+encodeURIComponent("${url}")+"&module=${module}&checked=${checked}&extId=${extId}&nodesLevel=${nodesLevel}&selectIds="+$("#${id}Id").val(), "<m:message bundle="common" key="common.select"/>${title}", 300, 420, {
			buttons:{"<m:message bundle="common" key="common.sure"/>":"ok", ${allowClear?"\"clear\":\"clear\", ":""}"<m:message bundle="common" key="common.close"/>":true}, submit:function(v, h, f){
				if (v=="ok"){
