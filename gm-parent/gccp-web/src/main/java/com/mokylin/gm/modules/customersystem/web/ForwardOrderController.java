...
@Controller
@RequiresUser
@RequestMapping(value = "/customersystem/forwardorder")
public class ForwardOrderController extends BaseController{

    @Resource
    private OrderService orderService;

    @RequiresPermissions(value = "forwardorder")
    ...
    public String list(@ModelAttribute(value = "queryCondition")QueryCondition queryCondition, Model model, HttpServletRequest request, HttpServletResponse response) {
        model.addAttribute("page", orderService.findOrderByStatus(new Page<WorkOrder>(request, response), queryCondition, OrderStatus.Forward));
        return "modules/customersystem/forwardorder/forwardList";
    }
}
