...
@Controller
@RequiresUser
@RequestMapping(value = "/customersystem/pendingorder")
public class PendingOrderController extends BaseController{

    @Resource
    private OrderService orderService;

    @RequiresPermissions(value = "pendingorder")
    ...
    public String list(@ModelAttribute(value = "queryCondition")QueryCondition queryCondition, Model model, HttpServletRequest request, HttpServletResponse response) {
        model.addAttribute("page", orderService.findOrderByStatus(new Page<WorkOrder>(request, response), queryCondition,OrderStatus.Pending));
        return "modules/customersystem/pendingorder/pendingList";
    }
}
