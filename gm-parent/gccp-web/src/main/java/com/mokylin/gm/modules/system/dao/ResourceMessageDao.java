...
public class ResourceMessageDao extends BaseGMDao{
    ...
    public ResourceMessage loadResourceBundle(ResourceMessage resource) {
        String sql = "SELECT `key`, `value`, locale, bundle, `comment` FROM t_resource_bundle where `key`=? and locale=? and bundle=?;";
        try {
            return (ResourceMessage)getJdbcTemplate().queryForObject(sql, new Object[]{resource.getKey(), resource.getLocale(),resource.getBundle()},new int[]{Types.VARCHAR, Types.VARCHAR, Types.VARCHAR}, new ResourceMessageRowMapper());
        } catch (Exception e){
     ...
     * @return
     */
    public boolean addResourceMessage(final ResourceMessage resource) {
        String sql = "insert into t_resource_bundle(locale, `bundle`, `key`, `value`) values(?, ?, ?, ?)";
        try{
            getJdbcTemplate().update(sql, new PreparedStatementSetter() {
                @Override
                public void setValues(PreparedStatement ps) throws SQLException {
                    ps.setString(1, resource.getLocale());
                    ps.setString(2, resource.getBundle());
                    ps.setString(3, resource.getKey());
                    ps.setString(4, resource.getValue());
                }
            });
            return true;
        } catch (Exception e) {
    ...
    public boolean updateResourceMessage(final ResourceMessage resource) {
        String sql = "update t_resource_bundle set `value`=? where locale=? and `bundle`=? and `key`=? ";
        try{
            getJdbcTemplate().update(sql, new PreparedStatementSetter() {
                @Override
                public void setValues(PreparedStatement ps) throws SQLException {
                    ps.setString(1, resource.getValue());
                    ps.setString(2, resource.getLocale());
                    ps.setString(3, resource.getBundle());
                    ps.setString(4, resource.getKey());
