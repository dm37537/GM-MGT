...
public class ResourceMessageService extends BaseGmService{
    ...
    public ResourceMessage findResourceMessage(ResourceMessage resource) {
        return resourceMessageDao.loadResourceBundle(resource);
    }
    ...
    public boolean addResourceMessage(ResourceMessage resource) {
        boolean ret = resourceMessageDao.addResourceMessage(resource);
        if(ret)
            ResourceManager.refreshResource();
        return ret;
    }

    public boolean updateResourceMessage(ResourceMessage resource) {
        boolean ret = resourceMessageDao.updateResourceMessage(resource);
        if(ret)
            ResourceManager.refreshResource();
        return ret;
    }
