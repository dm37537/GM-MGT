...
@Controller
@RequiresUser
@RequestMapping(value = "/customersystem/processedorder")
public class ProcessedOrderController extends BaseController{
    @Resource
    private OrderService orderService;

    @RequiresPermissions(value = "processedorder")
    ...
    public String list(@ModelAttribute(value = "queryCondition")QueryCondition queryCondition, Model model, HttpServletRequest request, HttpServletResponse response) {
        model.addAttribute("page", orderService.findOrderByStatus(new Page<WorkOrder>(request, response), queryCondition, OrderStatus.Processed));
        return "modules/customersystem/processedorder/processedList";
    }
}