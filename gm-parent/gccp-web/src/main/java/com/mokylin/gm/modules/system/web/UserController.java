...
public class UserController extends BaseController {
    ...
    public String modifyPwd(String oldPassword, String newPassword, Model model, HttpServletRequest request, HttpServletResponse response) {
        ...
        if(StringUtils.isNotBlank(oldPassword) && StringUtils.isNotBlank(newPassword)) {
            if(UserService.validatePassword(oldPassword, user.getPassword())) {
                if(userService.updatePassword(user.getUserName(), newPassword, oldPassword)) {
    ...
    public String gameSelect(String id, HttpServletRequest request, HttpServletResponse response) {
        if(StringUtils.isNotBlank(id)){
            request.getSession().setAttribute(GameUtils.CURRENT_GAME, id);
        }
