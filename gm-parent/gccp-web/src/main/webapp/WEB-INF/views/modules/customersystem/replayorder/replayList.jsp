...
<body>
<form:form id="searchForm" action="${ctx}/customersystem/replyorder" method="post" modelAttribute="queryCondition" class="breadcrumb form-search">
...
<table id="contentTable" class="table table-striped table-bordered table-condensed table-hover">
    ...
    <c:forEach items="${page.list}" var="order">
        <tr>
            <td><a href="${ctx}/customersystem/replyorder/replayOrderForm?id=${order.id}">${order.id}</a></td>
            <td>${order.type}</td>
            <td><a href="${ctx}/customersystem/replyorder/replayOrderForm?id=${order.id}">${order.title}</a></td>
            <td>${order.roleId}</td>
