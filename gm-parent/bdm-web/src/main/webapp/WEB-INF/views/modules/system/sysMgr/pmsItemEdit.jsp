...
<ul class="breadcrumb">
    ...
    <c:if test="${item.pms_item_parent_id!=0}"> <li class="active">编辑[<font color="red">${parent_item.pms_item_title}</font> ]的子权限</li></c:if>
    <c:if test="${item.pms_item_parent_id==0}"> <li class="active">编辑分类</li></c:if>
</ul>
        ...
        <div class="control-group">
            ...
            <div class="controls">
                <select name="pms_item_state">
                    <option value="0" <c:if test="${item.pms_item_state==0}"> selected="selected"</c:if>>正常</option>
                    <option value="-1" <c:if test="${item.pms_item_state==-1}"> selected="selected"</c:if>>禁用</option>
                </select><br />
            </div>
        </div>
    </c:if>
