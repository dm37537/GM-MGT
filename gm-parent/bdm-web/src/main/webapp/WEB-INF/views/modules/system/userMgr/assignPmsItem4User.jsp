        ...
        function onClick(event, treeId, treeNode, clickFlag) {
            if(treeNode.pms_item_source!=''){
                if(treeNode.pms_item_source.indexOf("?") != -1){
                    $("#content").attr("src","${ctx}/"+treeNode.pms_item_source+"&user_id=${user.id}&config_item_id="+treeNode.id+"&sys_id="+${sys.id});
                }else{
                    $("#content").attr("src","${ctx}/"+treeNode.pms_item_source+"?user_id=${user.id}&config_item_id="+treeNode.id+"&sys_id="+${sys.id});
                }
...
<body>
<h1>[<font color="red">${sys.system_name}</font>]中的私有权限</h1>
